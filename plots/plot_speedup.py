...
def plot(input_dir: str, output_dir: str) -> None:
    ...
    plt.figure()
    plt.plot(df["world_size"], t1_comm / df["total_ms_with_comm"], marker="o", label="with comm")
    plt.plot(df["world_size"], t1_no / df["total_ms_without_comm"], marker="o", label="without comm")
    plt.xlabel("processes")
    plt.ylabel("speedup")
    plt.title("Speedup")
    plt.legend()
    plt.tight_layout()
    plt.savefig(out / "03_speedup.png")
    plt.close()
